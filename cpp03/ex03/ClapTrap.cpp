...
ClapTrap::ClapTrap(void) : _name("Unnamed"), _hitPoints(10), _energyPoints(10), _attackDamage(0){
    std::cout << "[🆕]Hi! (ClapTrap) default constructor called" << std::endl;
}
ClapTrap::ClapTrap(std::string name) : _name(name), _hitPoints(10), _energyPoints(10), _attackDamage(0) {
    std::cout << "[🆕]Hi! (ClapTrap) " << _name << " parameter constructor called" << std::endl;
}

ClapTrap::~ClapTrap(void){
    std::cout << "[🆓]Bye!. (ClapTrap) destructor called " << std::endl;
}
