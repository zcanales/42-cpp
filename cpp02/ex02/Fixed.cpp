...
Fixed Fixed::operator+(Fixed const & raw)const {
	...
	//std::cout << "tem: " << temp.getRawBits() << std::endl;
	return (temp);
};
//	Importante, primero sumar y luego añadir con raw. Porque si lo añadimos desde el contructor se convierte a fractional
Fixed Fixed::operator-(Fixed const & raw) const{
